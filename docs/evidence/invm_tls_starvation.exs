...
defmodule Probe do
  ...
  def run(count, blocking) do
    ...
    # In-VM clients: connect first (the sender hands the server end off), then
    # drive a real TLS handshake from inside this VM - the freeze_repro topology
    # with the only difference being that the sender lives out of process.
    clients =
    ...
    Enum.each(clients, fn socket ->
      spawn(fn ->
        :ssl.connect(socket, [verify: :verify_none, server_name_indication: ~c"localhost"], 30_000)
      end)
    end)

    say(:in_vm_tls_handshakes_started)
    Process.sleep(500)
