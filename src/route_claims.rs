...
impl Claims {
    pub fn add(&mut self, pattern: &str, workload: &str) {
        if self.routes.get(pattern).is_some_and(|owners| owners.contains(workload)) {
            return;
        }
        ...
        self.routes.entry(pattern.to_string()).or_default().insert(workload.to_string());
    }

    pub fn retain_registrations(&mut self, assignments: &port_registry::LogicalAssignments) {
        let registered = |workload: &String| {
            assignments.contains_key(&(workload.clone(), "https".to_string()))
        };
    ...
    pub fn matching(&self, hostname: &str) -> Option<(&str, &BTreeSet<String>)> {
        self.routes.get_key_value(hostname)
            .or_else(|| route_pattern::matching_wildcard(hostname)
                .and_then(|pattern| self.routes.get_key_value(&pattern)))
            .map(|(pattern, owners)| (pattern.as_str(), owners))
    }

    pub fn conflict_count(&self) -> usize {
        self.routes.values().filter(|owners| owners.len() > 1).count()
    }
}
...
pub fn load_until(
    ...
    let document = port_registry::read_until(
        path, port_registry::RegistrySecurity::DerivedState, deadline,
    )?;
...
pub fn update_until(
    ...
    port_registry::update_if_changed_until(
        path, port_registry::RegistrySecurity::DerivedState, deadline, |document| {
            let mut claims = parse(document, !existed)?;
...
fn parse(document: &DocumentMut, allow_empty: bool) -> Result<Claims, String> {
    ...
    if document.get("version").and_then(|item| item.as_integer()) != Some(1) {
        return Err("ownership claims require version = 1; restore durable claims, do not discard them".into());
    }
    ...
    let routes = document.get("claims").and_then(|item| item.as_table())
    ...
    claims.saturated_workloads = parse_owners(
        document.get("saturated_workloads").and_then(|item| item.as_array()),
...
fn parse_owners(array: Option<&Array>, limit: usize) -> Result<BTreeSet<String>, String> {
    ...
    for item in array {
        let owner = item.as_str().ok_or_else(|| "ownership Workloads must be strings".to_string())?;
...
fn encode(claims: &Claims) -> DocumentMut {
    ...
    document["saturated_workloads"] = value(
        claims.saturated_workloads.iter().map(String::as_str).collect::<Array>(),
    );
...
pub fn print(path: &Path) -> Result<(), String> {
    ...
    for (pattern, owners) in claims.routes {
        let state = if owners.len() == 1 { "requires verification" } else { "conflict" };
        println!("{pattern} -> {} (https) [{state}; durable claim]", owners.into_iter().collect::<Vec<_>>().join(", "));
    }
    if !claims.saturated_workloads.is_empty() {
        println!("certificate_discovery blocked: claim_capacity_exhausted; remove saturated HTTPS registrations to release unrecorded claims");
    }
    Ok(())
}
...
mod tests {
    ...
    fn exact_claims_and_conflicts_survive_reload_until_registration_removal() {
        ...
        let claims = update_until(&path, None, |claims| claims.retain_registrations(&assignments)).unwrap();
        assert_eq!(claims.matching("specific.example.test").unwrap().1, &BTreeSet::from(["exact".into()]));
        assert_eq!(claims.matching("another.example.test").unwrap().0, "*.example.test");
    }
    ...
    fn capacity_never_evicts_exact_claims_and_saturation_is_durable() {
        ...
        assert_eq!(claims.saturated_workloads, BTreeSet::from(["overflow".into()]));
        let assignments = port_registry::LogicalAssignments::from([
            (("old".into(), "https".into()), 4101),
        ]);
        let claims = update_until(&path, None, |claims| claims.retain_registrations(&assignments)).unwrap();
    ...
    fn damaged_durable_state_is_never_discarded_or_rebuilt() {
        let directory = directory();
        let path = directory.path().join("route-claims.toml");
        update_until(&path, None, |claims| claims.add("exact.example.test", "exact")).unwrap();
        for invalid in ["", "not valid TOML [", "version = 99\n"] {
            fs::write(&path, invalid).unwrap();
            assert!(load_until(&path, None).is_err());
            assert!(update_until(&path, None, |claims| claims.add("*.example.test", "wild")).is_err());
