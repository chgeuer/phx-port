...
pub(crate) fn short_test_runtime(path: &Path) -> PathBuf {
...
mod platform {
    ...
    use nix::sys::socket::{
        AddressFamily, ControlMessage, MsgFlags, SockFlag, SockType, UnixAddr, getsockopt, recv,
    ...
    mod tests {
        ...
        #[test]
        fn full_accept_queue_obeys_connect_timeout() {
            ...
            let worker = thread::spawn(move || {
                let _ = sender.send(super::connect_endpoint(&path));
            });
            let result = receiver
                .recv_timeout(super::CONTROL_TIMEOUT + std::time::Duration::from_millis(500));
            drop(listener);
            drop(queued);
            worker.join().unwrap();
            let error = result
                .expect("full Unix accept queue outlived the connect deadline")
                .expect_err("a full accept queue must not connect");
        ...
        fn transfers_one_untouched_connected_stream_descriptor() {
            ...
            let endpoint = endpoint_path(
                EndpointIdentity::Production("contoso-web"),
                "https",
                Some(&crate::handoff::short_test_runtime(directory.path())),
            )
            .unwrap();
            let listener = socket(
                AddressFamily::Unix,
